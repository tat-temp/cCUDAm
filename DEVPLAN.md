...
dropping them changes no emitted byte. **The bodies are generated by extraction** —
`asm/tk/mkinc.py` copies whole blocks and diffs each one back against its origin line for line —
...
eight gained stall cycles, and that is the one part of this that is not free: a `FUNCTION` body
carries fixed control codes, and RCAsm parameters rename register indices only — they cannot reach
a stall count (`compiler.py:603` rejects a parameter whose name does not begin `R`/`UR`/`P`/`C`, and
...
stale register. Two of the six sit in 511×-per-batch loops: ~3,080 added stall cycles per batch
against ~655,000 instructions, and whether that is visible depends on whether another warp had
something to issue, which is not answerable offline.
...
**Six of the ten committed cubins are now older than `main.asm`** — `sufp`, `inv`, `walk`, `pts`,
`jump`, `loop`, plus `TestKernel.cubin` and `TestKernel_occ255.cubin` which derive from `loop`. They
are still the exact images that passed on hardware, so they are correct; they are just no longer
what the source says, which means **the ladder currently tests the old code**. Rebuilding needs the
full RCAsm path — a checkout with the four fixes and a taught sm_120 encoder repository — and until
that runs, no timing claim should be made about the stall change. `id`, `local`, `call` and `full`
are unaffected: those stages contain no copy block and their streams came out identical.
