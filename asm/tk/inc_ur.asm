...
//
// Every instruction line below started as a byte-for-byte copy out of Kernel02's
// asm/mod_mul.asm, asm/mod_sub.asm and asm/mod_inv.asm -- extracted mechanically and diffed
// back line for line, never retyped, because these are hand-scheduled control codes and one
// wrong character is a wrong answer on hardware rather than a build failure. Those three
// files are still in asm/ if a body here ever needs to be compared against its origin, but
// nothing reads them any more: build.sh feeds RCAsm main.asm and this file, and that is all.
//
...
// since this is 18 instructions against 1023 calls per thread.
    ...
    [B------:R-:W-:-:S06]    IADD3.X Ro6, Pt0, Pt2, RFirst6, ~URSecond6, ~RThird6, Pt0, Pt2  //was S01
    [B------:R-:W-:-:S06]    MOV Rt1, 0xFFFFFC2F                                            //was S01
