...
//@@HASHS_BEGIN
// ==== seed candidate HASH: getHash160_w2(prefix = 0x02 | (y1&1), x = x1 = PntX) ; hw2 -> R52 ====
// GpuCore.cu:190-204 hashes the batch's starting point BEFORE the suffix products. x = x1 = PntX
// (persistent R8-15; the hash owns R48..R106, clear of it, so x1 survives); prefix = 0x02 |
// (y1[0]&1) = 0x02 | (PntY0 & 1). Overlay-C is entirely dead here (suffix products not computed
...
//@@HASHP_BEGIN
// ==== + candidate HASH: getHash160_w2(prefix = 0x02 | (y3&1), x = px3 = PxN) ; hw2 -> R52 ====
// The lifted hash owns R48..R106 + UR6 (the +48 renumber of the verified getHash160_w2), has
// NO barriers, and is reached through the uCallH return pair. px3 is in PntX'? no -- in PxN;
// marshal PxN -> the hash's x-inputs R54..R61 (in-order: R54+k <- PxN_k, exactly the hashdump's
// e7..e0 map) and prefix (0x02|TmpA, TmpA = y3 parity) -> R52. PxN is dead after the candidate,
// so the hash clobbering R48..R106 is safe: the minus branch below reloads MulB/Lam/Sqr/MulA/
// MulR fresh, and Rinv(R32-39)/Dxi(R40-47)/PntX/PntY/COfs are all outside R48..R106. The copy
// idiom is the file's own alternating IMAD/MOV (two pipes, dual-issue). (Stage 3a: hw2 unused.)
...
//@@HASHM_BEGIN
// ==== - candidate HASH: getHash160_w2(prefix = 0x02 | (y3&1), x = px3 = PxN) ; hw2 -> R52 ====
// Identical to the + site (see HASHP): px3 in PxN, prefix from TmpA; marshal into R54..R61/R52
// and call the shared getHash160_w2 body. Dxi is already consumed by this branch, so R40..R47
// need not survive here either -- but the hash stays clear of them regardless. (3a: hw2 unused.)
    [B------:R-:W-:-:S02]    IADD3 R52, TmpA, 0x2, RZ
    [B------:R-:W-:-:S02]    IMAD R54, RZ, RZ, PxN0
    [B------:R-:W-:-:S02]    MOV  R55, PxN1
    [B------:R-:W-:-:S02]    IMAD R56, RZ, RZ, PxN2
...
inc_func SubMod256(RFirst=MulR, RSecond=PntY, Ro=MulA, Pt=0)
    [B------:R-:W-:-:S05]    LOP3.LUT TmpA, MulA0, 0x1, RZ, 0xc0, !PT
//@@HASHT_BEGIN
// ==== tail candidate HASH: getHash160_w2(prefix = 0x02 | (y3&1), x = px3 = PxN) ; hw2 -> R52 ====
// The tail is the minus branch alone, once per batch. Same marshal+call as HASHP/HASHM. (3a: unused.)
    [B------:R-:W-:-:S02]    IADD3 R52, TmpA, 0x2, RZ
    [B------:R-:W-:-:S02]    IMAD R54, RZ, RZ, PxN0
    [B------:R-:W-:-:S02]    MOV  R55, PxN1
    [B------:R-:W-:-:S02]    IMAD R56, RZ, RZ, PxN2
